...
// Define a local copy of jQuery
  ...
  jQuery.extend = jQuery.fn.extend = function () {
    ...
    let target = arguments[0] || {}; let i = 1; const { length } = arguments; let deep = false; let options; let name; let src; let
      copy;
      ...
      if ((options = arguments[i]) != null) {
        ...
        for (name in options) {
          ...
          if (deep && copy && (jQuery.isPlainObject(copy) || jQuery.isArray(copy))) {
            const clone = src && (jQuery.isPlainObject(src) || jQuery.isArray(src)) ? src
              : jQuery.isArray(copy) ? [] : {};
  ...
  const expando = `jQuery${now()}`; let uuid = 0; const
    windowData = {};
  ...
  jQuery.extend({
    ...
    data(elem, name, data) {
      ...
      let id = elem[expando]; const { cache } = jQuery; let
        thisCache;
    ...
    removeData(elem, name) {
      ...
      const id = elem[expando]; const { cache } = jQuery; const
        thisCache = cache[id];
  ...
  jQuery.fn.extend({
    data(key, value) {
      ...
      if (value === undefined) {
        let data = this.triggerHandler(`getData${parts[1]}!`, [parts[0]]);
  ...
  jQuery.extend({
    ...
    dequeue(elem, type) {
      ...
      const queue = jQuery.queue(elem, type); let
        fn = queue.shift();
  ...
  const rradiocheck = /radio|checkbox/;
    ...
    toggleClass(value, stateVal) {
      ...
      return this.each(function () {
        if (type === 'string') {
          ...
          let className; let i = 0; const self = jQuery(this);
          let state = stateVal;
          const classNames = value.split(rspace);
 ...
 */
  jQuery.event = {
    ...
    add(elem, types, handler, data) {
      ...
      const elemData = jQuery.data(elem);
      ...
      var eventHandle = elemData.handle; var
        eventHandle;
    ...
    remove(elem, types, handler, pos) {
      ...
      let ret; let type; let fn; let i = 0; let all; let namespaces; let namespace; let special; let eventType; let handleObj; let origType;
      const elemData = jQuery.data(elem);
    ...
    trigger(event, data, elem /* , bubbling */) {
      ...
      const handle = jQuery.data(elem, 'handle');
      if (handle) {
      ...
      if (!event.isPropagationStopped() && parent) {
        ...
        const { target } = event;
        let old;
        const isClick = jQuery.nodeName(target, 'a') && type === 'click';
        const special = jQuery.event.special[type] || {};
    ...
    handle(event) {
      let all; var handlers; let namespaces; let namespace; var
        events;
      ...
      var events = jQuery.data(this, 'events'); var
    ...
    fix(event) {
      ...
      if (event.pageX == null && event.clientX != null) {
        const doc = document.documentElement; const
          { body } = document;
    ...
    special: {
      ...
      live: {
        ...
        remove(handleObj) {
          let remove = true;
          const type = handleObj.origType.replace(rnamespaces, '');
  ...
  function liveHandler(event) {
    let stop; const elems = []; const selectors = []; const args = arguments;
    let related; let match; let handleObj; let elem; let j; let i; let l; let data;
    const events = jQuery.data(this, 'events');
 ...
 */
  (function () {
    ...
    let Sizzle = function (selector, context, results, seed) {
      ...
      let parts = []; let m; let set; let checkSet; let extra; let prune = true; const contextXML = isXML(context);
      let soFar = selector;
      ...
      while (expr && set.length) {
        for (const type in Expr.filter) {
          if ((match = Expr.leftMatch[type].exec(expr)) != null && match[2]) {
            const filter = Expr.filter[type]; var found; var item; const
              left = match[1];
            anyFound = false;
  ...
  const winnow = function (elements, qualifier, keep) {
  ...
  jQuery.fn.extend({
    find(selector) {
      const ret = this.pushStack('', 'find', selector); let
        length = 0;
    ...
    closest(selectors, context) {
      if (jQuery.isArray(selectors)) {
        const ret = []; let cur = this[0]; let match; const matches = {}; let
          selector;
      ...
      const pos = jQuery.expr.match.POS.test(selectors)
  ...
  const rxhtmlTag = /(<([\w:]+)[^>]*?)\/>/g;
  const rselfClosing = /^(?:area|br|col|embed|hr|img|input|link|meta|param)$/i;
  const rtagName = /<([\w:]+)/;
  const rtbody = /<tbody/i;
  const rhtml = /<|&#?\w+;/;
  const rnocache = /<script|<object|<embed|<option|<style/i;
  const rchecked = /checked\s*(?:[^=]|=\s*.checked.)/i; // checked="checked" or checked (html5)
  const fcloseTag = function (all, front, tag) {
  ...
  jQuery.fn.extend({
    ...
    replaceWith(value) {
      if (this[0] && this[0].parentNode) {
        ...
        return this.each(function () {
          const next = this.nextSibling; const
    ...
    domManip(args, table, callback) {
      let results; let first; const value = args[0]; const scripts = []; let fragment; let
        parent;
  ...
  function cloneCopyEvent(orig, ret) {
    ...
    ret.each(function () {
      ...
      const oldData = jQuery.data(orig[i++]); const curData = jQuery.data(this, oldData); const
  ...
  jQuery.each({
    ...
    jQuery.fn[name] = function (selector) {
      let ret = []; const insert = jQuery(selector);
  ...
  const styleFloat = jQuery.support.cssFloat ? 'cssFloat' : 'styleFloat';
  ...
  jQuery.extend({
    style(elem, name, value) {
      ...
      const style = elem.style || elem; const
        set = value !== undefined;
      ...
      if (!jQuery.support.opacity && name === 'opacity') {
        if (set) {
          ...
          const opacity = `${parseInt(value, 10)}` === 'NaN' ? '' : `alpha(opacity=${value * 100})`;
          const filter = style.filter || jQuery.curCSS(elem, 'filter') || '';
    ...
    css(elem, name, force, extra) {
      if (name === 'width' || name === 'height') {
        let val; const props = cssShow; const
    ...
    curCSS(elem, name, force) {
      ...
      if (!force && style && style[name]) {
        ...
        const { defaultView } = elem.ownerDocument;
        ...
        const camelCase = name.replace(rdashAlpha, fcamelCase);
        ...
        if (!rnumpx.test(ret) && rnum.test(ret)) {
          ...
          const { left } = style;
          const rsLeft = elem.runtimeStyle.left;
  ...
  if (jQuery.expr && jQuery.expr.filters) {
    jQuery.expr.filters.hidden = function (elem) {
      const width = elem.offsetWidth; const height = elem.offsetHeight;
      const skip = elem.nodeName.toLowerCase() === 'tr';
  ...
  let jsc = now();
  const rscript = /<script(.|\s)*?\/script>/gi;
  ...
  jQuery.fn.extend({
    show(speed, callback) {
      ...
      for (let i = 0, l = this.length; i < l; i++) {
        const old = jQuery.data(this[i], 'olddisplay');

        this[i].style.display = old || '';

        if (jQuery.css(this[i], 'display') === 'none') {
          const { nodeName } = this[i];
          var display;
      ...
      for (let j = 0, k = this.length; j < k; j++) {
    ...
    animate(prop, speed, easing, callback) {
      ...
      return this[optall.queue === false ? 'each' : 'queue'](function () {
        ...
        const hidden = this.nodeType === 1 && jQuery(this).is(':hidden');
        const self = this;

        for (p in prop) {
          const name = p.replace(rdashAlpha, fcamelCase);
  ...
  jQuery.extend({
    speed(speed, easing, fn) {
      const opt = speed && typeof speed === 'object' ? speed : {
  ...
  jQuery.fx.prototype = {
    ...
    cur(force) {
      ...
      const r = parseFloat(jQuery.css(this.elem, this.prop, force));
      ...
      if (gotoEnd || t >= this.options.duration + this.startTime) {
        ...
        if (done) {
          if (this.options.display != null) {
            ...
            // Reset the display
            const old = jQuery.data(this.elem, 'olddisplay');
  ...
  if ('getBoundingClientRect' in document.documentElement) {
    jQuery.fn.offset = function (options) {
      ...
      const box = elem.getBoundingClientRect(); const doc = elem.ownerDocument; const { body } = doc; const docElem = doc.documentElement;
      const clientTop = docElem.clientTop || body.clientTop || 0; const clientLeft = docElem.clientLeft || body.clientLeft || 0;
      const top = box.top + (self.pageYOffset || jQuery.support.boxModel && docElem.scrollTop || body.scrollTop) - clientTop;
      const left = box.left + (self.pageXOffset || jQuery.support.boxModel && docElem.scrollLeft || body.scrollLeft) - clientLeft;
  ...
  jQuery.offset = {
    initialize() {
      ...
      let table;
      let td;
      const bodyMarginTop = parseFloat(jQuery.curCSS(body, 'marginTop', true)) || 0;
    ...
    setOffset(elem, options, i) {
      ...
      const curTop = parseInt(jQuery.curCSS(elem, 'top', true), 10) || 0;
      const curLeft = parseInt(jQuery.curCSS(elem, 'left', true), 10) || 0;
  ...
  jQuery.fn.extend({
    position() {
      ...
      const parentOffset = /^body|html$/i.test(offsetParent[0].nodeName) ? { top: 0, left: 0 } : offsetParent.offset();
